...
from bianbt.reports.matrix import render_matrix_research_report
    ...
    for period, month in sorted(summary["months"].items()):
        for factor, values in sorted(month["factors"].items()):
            ...
            for cost, metrics in sorted(values["fast_matrix"].items()):
                ...
                (destination / f"{run_id}.html").write_text(page, encoding="utf-8")
...
def render_factor_study_reports(
    ...
    quick.write_text(_quick_report(summary), encoding="utf-8")
    matrix.write_text(_matrix_report(summary), encoding="utf-8")
...
<a class="nav-card" href="summary.json"><strong>机器可读汇总</strong><span>供批处理、二次分析和精确复现使用</span></a></div></section>"""
    landing.write_text(
        _page(str(summary["study_id"]), "RESEARCH WORKFLOW", body), encoding="utf-8"
    )
    return {"landing": landing, "quick_research": quick, "fast_matrix": matrix}
...
<div class="table-wrap"><table id="results"><thead><tr>{headers}</tr></thead><tbody>{''.join(rows)}</tbody></table></div></section>"""
    ...
    report.write_text(
        _page(str(summary.get("title", study_id)), "GTJA191 QUICK RESEARCH", body,
