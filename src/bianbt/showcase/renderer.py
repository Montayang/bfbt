...
<div class="card-top"><span>{_escape(run['label'])}</span><span class="badge {'warn' if qualified else 'ok'}">{badge}</span></div>
...
<div><dt>成交</dt><dd>{run['trade_count']}</dd></div></dl>
        ...
        provenance_rows.append(
            f"""<article class="evidence-card"><div><span class="eyebrow">{_escape(run['label'])}</span>
...
<nav aria-label="Evidence links"><a href="{_escape(links['report'])}">深度报告</a><a href="{_escape(links['config'])}">配置</a><a href="{_escape(links['metrics'])}">指标</a><a href="{_escape(links['manifest'])}">Manifest</a><a href="{_escape(links['warnings'])}">警告</a></nav></article>"""
        ...
        warning_blocks.append(
            f"<details><summary>{_escape(run['label'])} · {len(warnings)} 条</summary>"
            + (
        ...
        paths.append(
            f"<polyline points=\"{_margin_polyline(run['opening_margin_trajectory'])}\" fill=\"none\" stroke=\"{color}\" stroke-width=\"3\" vector-effect=\"non-scaling-stroke\"><title>{_escape(run['label'])}</title></polyline>"
        )
...
<title>{_escape(presentation['title'])} · bianbt Showcase</title>
<style>
...
<header class="hero"><span class="eyebrow">BIANBT · VERIFIED RESEARCH SHOWCASE</span><h1>{_escape(presentation['title'])}</h1><p class="subtitle">{_escape(presentation['subtitle'])}</p><span class="boundary">离线研究系统 · 无账户连接 · 不执行真实订单</span><nav class="quick-nav"><a href="#workflow">工作流</a><a href="#intent">研究冻结</a><a href="#results">多期结果</a><a href="#audit">审计证据</a></nav></header>
...
<footer>由 bianbt 从不可变回测产物确定性生成。This is historical simulation evidence, not investment advice.</footer>
</main></body></html>"""
