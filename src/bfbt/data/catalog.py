...
from bfbt.data.schemas import SchemaDefinition, list_schema_definitions
    ...
    def _require_version(
        ...
        try:
            ...
            raise CatalogNotInitializedError(
                f"not a bfbt catalog: {self.path}"
            ) from exc
        if row is None:
            raise CatalogNotInitializedError(f"not a bfbt catalog: {self.path}")
