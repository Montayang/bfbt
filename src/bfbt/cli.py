...
from bfbt.config.bundle import RunReadinessError
from bfbt.config.factor import FactorConfig
from bfbt.config.fingerprint import canonical_config, config_fingerprint
from bfbt.config.loader import (
    ConfigLoadError,
    ConfigPaths,
    load_config_bundle,
    project_root,
...
from bfbt.engine.vectorized import run_vectorized_backtest
...
from bfbt.engine.fast_matrix.target_schedule import (
    TargetScheduleError,
...
from bfbt.performance.spool import SpoolError, cleanup_stale_workspaces
from bfbt.portfolio.constraints import construct_portfolio
from bfbt.research.evaluator import evaluate_factor
from bfbt.reports.research_study import (
    ResearchStudyReportError,
    render_factor_study_reports,
)
from bfbt.reports.locales import variant_path
from bfbt.reports.renderer import ReportError, render_report_from_artifacts
from bfbt.data.sources.base import ArchiveDiscoveryRequest, SourceError
...
from bfbt.showcase.doctor import doctor as run_doctor
from bfbt.showcase.models import (
...
def showcase_build(
    ...
    typer.echo(f"page_en={variant_path(page, 'en')}")
    typer.echo(f"page_zh_cn={variant_path(page, 'zh-CN')}")
...
def rebuild_report(
    ...
    typer.echo(f"report_en={variant_path(rendered, 'en')}")
    typer.echo(f"report_zh_cn={variant_path(rendered, 'zh-CN')}")
