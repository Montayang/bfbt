...
from bfbt.reports.locales import add_language_switch, html_variants, variant_path
from bfbt.showcase.models import ShowcaseRunReference, ShowcaseSpec
from bfbt.showcase.renderer import render_showcase
...
def inspect_showcase(
    ...
    *,
    ...
    payload: dict[str, Any] = {
        "evidence_version": "bfbt-showcase-evidence/v1",
...
def build_showcase(
    ...
    *,
    ...
    rendered = html_variants(render_showcase(evidence))
    default_path = destination / "index.html"
    english_path = variant_path(default_path, "en")
    chinese_path = variant_path(default_path, "zh-CN")
    english = add_language_switch(
        rendered["en"],
        locale="en",
    )
    chinese = add_language_switch(
        rendered["zh-CN"],
        locale="zh-CN",
    )
    _atomic_text(destination / "evidence.json", evidence_text)
    _atomic_text(default_path, english)
    _atomic_text(english_path, english)
    _atomic_text(chinese_path, chinese)
    return default_path, evidence
